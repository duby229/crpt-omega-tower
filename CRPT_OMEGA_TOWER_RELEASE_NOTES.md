...
- **The status system, fully discharged.** `Status` (Lω.8.D3): five provenance statuses — Imported, Specialized, Reframed, Derived, Novel — each with a mandatory body obligation, as part of the Spec-Sub self-model. Migration pass 1 (21 Imported→Specialized, 8 Novel→Derived) was followed by the deep adjudication: 56 further Novel→Derived (instance theorems, restatement corollaries, commentary remarks), 17 Reframed→Specialized (pure domain restrictions), 1 Reframed→Derived. **Every obligation is discharged:** 387 Novelty Notes (basis credited, the new content stated, non-derivability argued per type — axioms cite their L1.6 independence witnesses), 14 Reframing Notes (11 newly authored — `Fiber`, `Conv`, `ProjMor`, `ProjSys-Cat`, PA-Bisim, `Fix`, `d_M`, `Ker-Cong`, `PV-WF`, `GI`, `Faith-Emb` — joining the ρ_M/PV-Obs/Inst-Emb flagships), 66 Derivation chains, 38 Specialized clauses carried by Source lines (discharge conventions stated in `Status`). Final distribution: 387 Novel / 66 Derived / 53 Imported / 38 Specialized / 14 Reframed — the genuine novelty isolated, every credit explicit. The obligation notes contribute 861 new cross-references (2,854 validated hyperlinks at release).
